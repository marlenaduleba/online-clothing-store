import {
  ...
  describe('Token Model', () => {
    ...
    describe('saveRefreshToken', () => {
      it('should generate and save a new refresh token', async () => {
        const mockUserId = 1;
        ...
        const result = await saveRefreshToken(mockUserId);
  
        expect(crypto.randomBytes).toHaveBeenCalledWith(64);
        expect(query).toHaveBeenCalledWith(
          'INSERT INTO refresh_tokens (user_id, token, expires_at) VALUES ($1, $2, $3)',
          [mockUserId, 'mockedToken', mockExpiresAt]
        );
    ...
    describe('revokeRefreshToken', () => {
      it('should revoke (delete) the refresh token', async () => {
        ...
        expect(query).toHaveBeenCalledWith('DELETE FROM refresh_tokens WHERE token = $1', [mockToken]);
      });
    });
    ...
    describe('blacklistToken', () => {
      ...
      it('should throw an error if token cannot be blacklisted', async () => {
        ...
        (query as jest.Mock).mockRejectedValueOnce(new Error('Failed to blacklist token'));
  
        await expect(blacklistToken(mockToken)).rejects.toThrow('Could not blacklist token');
      });
    });
    ...
    describe('isTokenBlacklisted', () => {
      it('should return true if the token is blacklisted', async () => {
        ...
        expect(query).toHaveBeenCalledWith('SELECT 1 FROM token_blacklist WHERE token = $1', [mockToken]);
      ...
      it('should throw an error if blacklist status cannot be checked', async () => {
        ...
        // Mocking query to throw an error
        (query as jest.Mock).mockRejectedValueOnce(new Error('Failed to check blacklist'));
  
        await expect(isTokenBlacklisted(mockToken)).rejects.toThrow('Could not check token blacklist');
      });
    });
    ...
    describe('getRefreshTokenData', () => {
      it('should return user ID associated with a valid refresh token', async () => {
        ...
        (query as jest.Mock).mockResolvedValueOnce({ rows: [{ user_id: mockUserId }] });
